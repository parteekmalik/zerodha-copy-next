...
export function OrderTypeDiv({
  ...
  return (
    <div className={twMerge("flex w-full items-center justify-center gap-2", className)}>
      <CheckBox data={{ isSelected: !isMarketOrder, type: "LIMIT" }} clickHandler={() => setFormdata(false)} />
      <CheckBox data={{ isSelected: isMarketOrder, type: "MARKET" }} clickHandler={() => setFormdata(true)} />
    </div>
  );
}
export function CheckBox({
  ...
  return (
    <div className="flex cursor-pointer items-center justify-center gap-1" onClick={clickHandler}>
      <div
        className={twMerge(
      ...
      >
        {isSelected && <div className="h-[6px] w-[6px] rounded-full bg-blue-500 "></div>}
      </div>
      <div>{type}</div>
    </div>
  );
