...
const useSocket = (
  ...
  const [lastMessage, setLastmessage] = useState<
    Order | BACKEND_SERVER_UPDATE_PAYLOAD | null
  >(null);
  const [backendServerConnection, setbackendServerConnection] =
    useState<BACKEND_SERVER_UPDATE_PAYLOAD>("disconneted");
  useEffect(() => {
    ...
    socketInstance.on(AUTHENTICATION, (msg: AUTHENTICATION_PAYLOAD) =>
      console.log(AUTHENTICATION, msg),
    );
    socketInstance.on(
      BACKEND_SERVER_UPDATE,
      (msg: BACKEND_SERVER_UPDATE_PAYLOAD) => setbackendServerConnection(msg),
    );
