...
import { type Tsymbol24hr } from "../../_contexts/LiveData/BinanceWS";
import { websocketService } from "../../_contexts/LiveData/BinanceWSContextComponent";
import { type RootState } from "../store";

const subsciptionsMddleware: ThunkMiddleware = (Store) => (next) => (action) => {
  next(action);
...
export const getLast24hrData = async (subscriptions: string[]) => {
  ...
  const subSymbol = JSON.stringify(subscriptions.map((item) => item.split("@")[0]?.toUpperCase()));
  // console.log("url", url);
  return await axios
    .get(url + subSymbol)
