...
import { type convertIntoAvg } from "./getOpenPositions";
...
export default async function getCurrentPositionDetails(
  ...
  const currentPrices = assetNamesList.length ? await getLast24hrData(assetNamesList) : [];
  const data = list.map((asset) => {
    const currentTotal =
      Number(currentPrices.find((i) => i.symbol === asset.name + "USDT")?.curPrice ?? 0) * asset.quantity;
